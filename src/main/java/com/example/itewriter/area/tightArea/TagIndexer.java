...
public class TagIndexer {
    public ObservableValue<SimpleVariation> createBinding(Registry.Tag tag) {
        return Bindings.createObjectBinding(() -> tag.getAllSimpleVariationsProperty().get(tagIndices.get(tag)), tagIndices);
    }
    TagIndexer(Registry registry) {
        registry.allTags.addListener((SetChangeListener.Change<? extends Registry.Tag> change) -> {
            if (change.wasAdded()) {
                ...
                if (tag.getAllSimpleVariationsProperty().isEmpty()) tagIndices.put(change.getElementAdded(), -1);
    ...
    public void setIndex(Registry.Tag tag, Integer index) {
        if (index >= 0 && index < tag.getAllSimpleVariationsProperty().size()) tagIndices.put(tag, index);
